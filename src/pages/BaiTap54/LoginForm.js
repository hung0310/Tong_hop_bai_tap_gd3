...
import { postDataLogin } from '../../Apis/getDataAPI'; 
...
const LoginPage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const accessToken = await postDataLogin(values.username, values.password); 
        if (accessToken) {
          ...
          <div className='col-lg-6 col-md-12'>
            <form onSubmit={formik.handleSubmit}>
              ...
              <div className='mt-5'>
                <span className="p-float-label">
                  <InputText
                    id="username"
                    name="username"
                    className={`w-full mb-3 ${formik.touched.username && formik.errors.username ? 'p-invalid' : ''}`}
                    placeholder=" "
                ...
                <span className="p-float-label mt-3">
                  <InputText
                    ...
                    name="password"
                    type="password"
                    className={`w-full mb-3 ${formik.touched.password && formik.errors.password ? 'p-invalid' : ''}`}
                    placeholder=" "
              ...
              <div className='d-flex justify-content-center align-items-center mt-2'>
                ...
                <Link to="/signin" className='px-2' style={{ color: "var(--foundation-orange, #F4841F)" }}>
                  Đăng ký
                </Link>
              </div>
            </form>
